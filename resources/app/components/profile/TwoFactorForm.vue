...
<template>
  <div class="rounded-lg border bg-card p-6 space-y-4">
    <div>
      <h3 class="text-base font-semibold">{{ t('pages.profile.twoFactorTitle') }}</h3>
      <p class="text-sm text-muted-foreground mt-1">{{ t('pages.profile.twoFactorDescription') }}</p>
    </div>

    <!-- Status badge -->
    <div class="flex items-center gap-2">
      <span class="text-sm text-muted-foreground">{{ t('pages.profile.twoFactorStatus') }}:</span>
      <Badge v-if="step === 'enabled'" variant="default">{{ t('pages.profile.twoFactorEnabled') }}</Badge>
      <Badge v-else variant="secondary">{{ t('pages.profile.twoFactorDisabled') }}</Badge>
    </div>

    <!-- Step: disabled -->
    <div v-if="step === 'disabled'">
      <Button :disabled="loading" @click="startEnable">
        {{ t('pages.profile.twoFactorEnable') }}
      </Button>
    </div>
    ...
    <!-- Step: enabled -->
    <div v-else-if="step === 'enabled'" class="flex flex-wrap gap-2">
